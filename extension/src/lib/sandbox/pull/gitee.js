 ...
 * @param {string} branch
 */
export async function pullGiteeSandbox(credential, apiBase, owner, repo, branch) {
  const tokenQuery = `access_token=${encodeURIComponent(credential.token)}`;

  const commitsUrl = `${apiBase}/repos/${owner}/${repo}/commits?sha=${encodeURIComponent(branch)}&per_page=1&${tokenQuery}`;
  const commitsRes = await fetch(commitsUrl, { headers: { Accept: 'application/json' } });
  ...
  const archiveBuffer = await downloadArchive(
    `${apiBase}/repos/${owner}/${repo}/zipball/${encodeURIComponent(latest.sha)}?${tokenQuery}`,
  );

  const contentsUrl = `${apiBase}/repos/${owner}/${repo}/contents/?ref=${encodeURIComponent(branch)}&${tokenQuery}`;
  const contentsRes = await fetch(contentsUrl, { headers: { Accept: 'application/json' } });
