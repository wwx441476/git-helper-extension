...
import { replaceGithubDirectoryDirect } from '../src/lib/github/path-replace.js';
import { readZipUploadFilesFromPath } from './zip-from-path.js';

function printHelp() {
  console.log(`GitHub 目录替换 CLI

用法:
  GITHUB_TOKEN=xxx node scripts/github-path-replace.mjs [选项]

选项:
  --zip <path>       本地 ZIP 包路径（必填）
  --api-base <url>   GitHub API 根地址
  --owner <name>     仓库 owner
  --repo <name>      仓库名
  --branch <name>    分支名
  --target <path>    远程目标目录
  --exclude <paths>  排除路径
  --message <text>   提交说明
  --token <token>    Token（或环境变量 GITHUB_TOKEN）
  -h, --help         显示帮助
`);
}
...
async function main() {
  ...
  const token = String(opts.token || process.env.GITHUB_TOKEN || '').trim();
  const zipPath = String(opts.zip || '').trim();
  const relativeFiles = await readZipUploadFilesFromPath(zipPath);

  const result = await replaceGithubDirectoryFromDirect({
    apiBase: String(opts.apiBase || 'https://api.github.com').trim(),
    token,
  ...
  console.log(`完成 · 删除 ${result.deletedCount} · 上传 ${result.createdCount} · ${result.commitSha}`);
